...
interface BefundEditFormProps {
  record: DiagnoseRecord
  patientId: string
}

export function BefundEditForm({ record, patientId }: BefundEditFormProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      hauptdiagnose: {
        icd10: record.hauptdiagnose.icd10 ?? null,
        sicherheitsgrad: record.hauptdiagnose.sicherheitsgrad,
        freitextDiagnose: record.hauptdiagnose.freitextDiagnose ?? "",
        freitextNotiz: record.hauptdiagnose.freitextNotiz ?? "",
      },
      nebendiagnosen: record.nebendiagnosen.map((n) => ({
        icd10: n.icd10 ?? null,
        sicherheitsgrad: n.sicherheitsgrad,
        freitextDiagnose: n.freitextDiagnose ?? "",
        freitextNotiz: n.freitextNotiz ?? "",
      })),
      therapieziel: record.therapieziel ?? "",
      prognose: record.prognose ?? "",
      therapiedauer_wochen: record.therapiedauer_wochen ?? null,
    },
  })
  ...
  async function submitForm(data: BefundFormValues, status: "entwurf" | "abgeschlossen") {
    ...
    try {
      const res = await fetch(`/api/patients/${patientId}/diagnoses/${record.id}`, {
        method: "PATCH",
      ...
      if (!res.ok) {
        if (res.status === 403) {
          setServerError("Zugriff verweigert. Nur der Ersteller oder ein Admin darf diesen Befund bearbeiten.")
        } else if (res.status === 409) {
          setServerError("Dieser Befund ist abgeschlossen und kann nicht mehr bearbeitet werden.")
        } else {
  ...
  const onSaveDraft = handleSubmit(async (data) => {
    ...
    try {
      ...
      if (ok) {
        toast.success("Befundbericht gespeichert.")
        router.push(`/os/patients/${patientId}/befund/${record.id}`)
        router.refresh()
      }
    } finally {
    ...
    <form className="space-y-10" noValidate>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Textarea
            ...
            rows={5}
            placeholder="z.B. Palpation der LWS: Druckschmerz L4/L5..."
      ...
      <div className="space-y-4">
        ...
        <Textarea
          id="therapieziel"
          rows={3}
          placeholder="z.B. Schmerzreduktion auf NRS ≤ 3, Wiederherstellung voller Schulterbeweglichkeit..."
